...
<body>
	...
	<div class="container">
		<div class="row">
										...
										<div class="row">
											...
											<div class="col-xs-4 col-md-3">
												...
												<c:if test="${!empty username && listArticle.username!=username}">
													<!-- 每一次遍历手链信息时初始化attention_record为“false” -->
													<c:set var="attention_record" value="false"></c:set>
													<!-- 遍历关注表信息 -->
													<c:forEach var="attention" items="${attention }">
														<!-- 判断该手链对应的用户是否被关注，如果是，则将“true”保存到"attention_record"中--->
										...
										<div class="row" style="position: relative;top: -10px;">
											<div class="col-xs-10 col-md-6">
												...
												<div class="col-xs-7 col-md-6">
													...
													<c:if test="${!empty username && listArticle.username!=username }">
														<!-- 每一次遍历手链信息时初始化collect_record为“false” -->
														<c:set var="collect_record" value="false"></c:set>
														<!-- 遍历收藏表信息 -->
														<c:forEach var="collect" items="${collect }">
															<!-- 判断该手链是否被收藏，如果是，则将“true”保存到"collect_record"中--->
													...
													<!-- 用户未登录才显示文字收藏 && 如果不是登录用户本人所发手链信息，则显示文字收藏 -->
													<c:if test="${empty username && listArticle.username!=username }">
														<small>登录后可收藏</small>
													</c:if>
												</div>
											...
											<div class="row" style="position: relative; margin-top: -10px;">
												<div class="col-md-12">
													...
													<p>${comment_list.pcontent}</p>
                                                            <input type="hidden" name="pid" value="${comment_list.pid }">
														   <input type="hidden" name="${APP_PATH }" value="${APP_PATH }" id="myPath">
                                                           <button type="submit" style="border: none;background-color: #ffffff;"  onclick="addaggree(${comment_list.pid})">
														<samp title="点赞" class="glyphicon glyphicon-thumbs-up"></samp>
													</button>
												</div>
											</div>
										</c:forEach>
			...
			<div id="content_right">
				...
				<c:forEach var="articlePerson" items="${articlePerson}">
					<%--文章数量大于3--%>
						<div class="row">
							<%--TODO --%>
							<div class="col-md-12">
								<a href="${APP_PATH }/userController/getOthers?userid=${articlePerson.userid}">${articlePerson.username}</a>
	...
	<script type="application/ecmascript">
		function addaggree(fid) {
			 $.ajax({
			 type : 'get',
			 url : '${APP_PATH }/common/addAgree',
			 data:{'pid':fid},
